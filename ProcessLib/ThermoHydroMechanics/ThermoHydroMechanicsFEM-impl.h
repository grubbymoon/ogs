...
void ThermoHydroMechanicsLocalAssembler<ShapeFunctionDisplacement,
  ...
  local_rhs.template segment<displacement_size>(displacement_index).noalias() +=
      Kup * p;
}
...
ThermoHydroMechanicsLocalAssembler<ShapeFunctionDisplacement,
                                   ShapeFunctionPressure, IntegrationMethod,
                                   DisplacementDim>::
    getIntPtDarcyVelocity(const double t, GlobalVector const &current_solution,
  ...
  auto cache_matrix = MathLib::createZeroedMatrix<
      Eigen::Matrix<double, DisplacementDim, Eigen::Dynamic, Eigen::RowMajor>>(
...
void ThermoHydroMechanicsLocalAssembler<ShapeFunctionDisplacement,
  ...
  assembleWithJacobianForDeformationEquations(
      ...
      local_Jac_data, local_coupled_solutions);
}
