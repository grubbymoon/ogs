...
public:
  ...
  void operator()(std::size_t const id, MeshLib::Element const &mesh_item,
    ...
    if (it != _builder.end()) {
      ...
      OGS_FATAL("You are trying to build a local assembler for an unknown mesh "
                "element type (%s)."
...
private:
  ...
  static LADataBuilder makeLocalAssemblerBuilder(std::true_type *) {
    ...
    return [](MeshLib::Element const &e, std::size_t const local_matrix_size,
