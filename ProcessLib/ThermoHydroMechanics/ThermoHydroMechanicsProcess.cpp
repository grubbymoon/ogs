...
void ThermoHydroMechanicsProcess<DisplacementDim>::initializeConcreteProcess(
  ...
  _secondary_variables.addSecondaryVariable(
      "sigma_xx", makeExtrapolator(1, getExtrapolator(), _local_assemblers,
                                   &LocalAssemblerInterface::getIntPtSigmaXX));

  _secondary_variables.addSecondaryVariable(
      "sigma_yy", makeExtrapolator(1, getExtrapolator(), _local_assemblers,
                                   &LocalAssemblerInterface::getIntPtSigmaYY));

  _secondary_variables.addSecondaryVariable(
      "sigma_zz", makeExtrapolator(1, getExtrapolator(), _local_assemblers,
                                   &LocalAssemblerInterface::getIntPtSigmaZZ));

  _secondary_variables.addSecondaryVariable(
      "sigma_xy", makeExtrapolator(1, getExtrapolator(), _local_assemblers,
...
void ThermoHydroMechanicsProcess<
  ...
  initializeProcessBoundaryConditionsAndSourceTerms(*_local_to_global_index_map,
...
void ThermoHydroMechanicsProcess<DisplacementDim>::assembleConcreteProcess(
  ...
  std::vector<std::reference_wrapper<NumLib::LocalToGlobalIndexMap>> dof_table =
...
void ThermoHydroMechanicsProcess<DisplacementDim>::
  ...
  if (_use_monolithic_scheme) {
    DBUG("Assemble the Jacobian of ThermoHydroMechanics for the monolithic"
         " scheme.");
    dof_tables.emplace_back(*_local_to_global_index_map);
  } else {
    // For the staggered scheme
    if (_coupled_solutions->process_id == 0) {
      DBUG("Assemble the Jacobian equations of liquid fluid process in "
           "ThermoHydroMechanics for the staggered scheme.");
    } else {
      DBUG("Assemble the Jacobian equations of mechanical process in "
           "ThermoHydroMechanics for the staggered scheme.");
    }
  ...
  GlobalExecutor::executeMemberDereferenced(
      ...
      _local_assemblers, dof_tables, t, x, xdot, dxdot_dx, dx_dx, M, K, b, Jac,
      _coupled_solutions);
...
ThermoHydroMechanicsProcess<DisplacementDim>::getDOFTableForExtrapolatorData()
    const {
