...
std::unique_ptr<Process> createThermoHydroMechanicsProcess(
  ...
  } else // staggered scheme.
  {
    ...
    for (auto const &variable_name : {"temperature"s,"pressure"s, "displacement"s}) {
  ...
  if (variable_u->getNumberOfComponents() != DisplacementDim) {
    OGS_FATAL("Number of components of the process variable '%s' is different "
              "from the displacement dimension: got %d, expected %d",
              variable_u->getName().c_str(),
              variable_u->getNumberOfComponents(), DisplacementDim);
  }

  DBUG("Associate pressure with process variable \'%s\'.",
       variable_p->getName().c_str());
  if (variable_p->getNumberOfComponents() != 1) {
    OGS_FATAL("Pressure process variable '%s' is not a scalar variable but has "
              "%d components.",
              variable_p->getName().c_str(),
  ...
  DBUG("Use \'%s\' as fluid volumetric thermal expansion coefficient "
       "parameter.",
  ...
  Eigen::Matrix<double, DisplacementDim, 1> specific_body_force;
  {
    ...
    if (b.size() != DisplacementDim) {
      OGS_FATAL("The size of the specific body force vector does not match the "
